...
def tutorial_create_root() -> py_trees.behaviour.Behaviour:
    ...
    send_goal=prototype_actions.SendCoordinates.Goal()
    send_goal.x=1.0
    send_goal.y=1.0
    feed=prototype_actions.SendCoordinates.Feedback()
    scan_rotate = py_trees_ros.actions.ActionClient(
        name="Rotate",
        action_type=prototype_actions.SendCoordinates,
        action_name="sendcoordinates",
        action_goal=send_goal,  # noqa
        generate_feedback_message=lambda msg: "{:.2f}%%".format(feed.current_x)
    )
